...
export default class GameScene extends Phaser.Scene {
  ...
  openBuildMenu(pad) {
    ...
    const radius = 95;
    const angles = types.map((_, i) => Phaser.Math.DegToRad(-152 + i * (300 / (types.length - 1))));
    ...
    types.forEach((type, i) => {
      ...
      const hasSprite = cfg.spriteKey && this.textures.exists(cfg.spriteKey);
      let btn;
      if (hasSprite) {
        btn = this.add.sprite(bx, by, cfg.spriteKey).setInteractive();
        const scale = 70 / btn.height;
        btn.setScale(scale);
      } else {
        btn = this.add.circle(bx, by, 28, cfg.color, 0.2)
      ...
      const refresh = () => {
        ...
        if (hasSprite) {
          btn.setAlpha(can ? 1 : 0.5).setTint(can ? 0xffffff : 0x555555);
        } else {
      ...
      // Base scale might be different if it's a sprite
      const baseScale = hasSprite ? (60 / btn.height) : 1;
      ...
      btn.on("pointerout", () => { btn.setScale(baseScale); this.events.emit("hideTooltip"); });
      btn.on("pointerdown", () => {
  ...
  fireOrbital(x, y) {
    ...
    // THE DROP BEAM
    const dropBeam = this.add.rectangle(x, -500, 50, 1000, 0xffffff).setDepth(35);
    dropBeam.setStrokeStyle(12, 0xc90be3, 0.8); // Adds the glowing neon purple edge
    this.tweens.add({
      targets: dropBeam,
      y: y - 500, // Mathematically puts the very bottom tip of the beam exactly at 'y'
      duration: 200,
      ease: "Linear",
      onComplete: () => {

        // As soon as the beam hits the ground, fade it out instantly
        this.tweens.add({
          targets: dropBeam,
          scaleX: 0,
          alpha: 0,
          duration: 200,
          onComplete: () => dropBeam.destroy()
        });

        // THE IMPACT
        this.cameras.main.shake(280, 0.011);
        AudioManager.playSFX("sfx_orbital");
        ...
        if (this.anims.exists("laser_fire")) {
          // Offset by -96 so the explosion sits perfectly on the ground!
          const laser = this.add.sprite(x + 2.5, y - 20, "orbital_laser").setDepth(36);
          laser.play("laser_fire");
          laser.once("animationcomplete", () => laser.destroy());
        }
  ...
  orbitalCooldownProgress() {
    ...
    return Math.min(this._orbitalCooldownTimer / this.orbitalCooldown, 1);
  }
  ...
  update(time, delta) {
    ...
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      if (!e.update(dt)) { this.damageBase(e.baseDamage, e.typeConfig); e.destroy(); this.enemies.splice(i, 1); }
      else if (e.isDead() && !e._dead) { this.killEnemy(e); }
    }
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      const p = this.projectiles[i];
      if (p.update(dt)) { p.destroy(); this.projectiles.splice(i, 1); }
    }

    this.waveManager.update(dt);

    if (!this.orbitalReady) {
      this._orbitalCooldownTimer += dt;
      if (this.orbitalCooldownProgress() >= 1) {
        this.orbitalReady = true;
  ...
  killEnemy(enemy) {
    ...
    this.tweens.add({
      targets: popup, y: popup.y - 26, alpha: 0, duration: 680,
    ...
    enemy.playDeath(() => {
      ...
      this.tweens.add({
        targets: ex, radius: 22, alpha: 0, duration: 250,
  ...
  damageBase(amount, typeConfig) {
    ...
    this.tweens.add({
      targets: flash, alpha: 0, duration: 320 + amount * 40,
    ...
    this.tweens.add({
      targets: dmgTxt, y: dmgTxt.y - 40, alpha: 0, duration: 900,
